...
def load_data_for_task(task: str = 'orm', version="v1"):
    if task in ["orm", "prm", "hrm"]:
        eval_data_path = f"dataset/self_training_{version}_policy/can_use_{task}_eval.jsonl"
    else:
...
def preprocess_function(examples, tokenizer):
    ...
    # Shift labels by one token (for causal LM)
    labels = model_inputs["input_ids"].copy()
    labels = labels[1:] + [tokenizer.pad_token_id]  # Shift by one and pad the last token
...
def main(task, version):
    _, tokenizer = load_model("models/Qwen2.5-Math-7B-Instruct")

    save_best_path = f"sf_best_policy_model/{task}/{version}"
    ...
    # Load all models
    base_path = f"outputs/self_training_{version}_policy_model/{task}/Qwen2.5-Math-7B-Instruct"
